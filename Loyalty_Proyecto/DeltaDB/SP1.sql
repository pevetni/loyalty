USE [DIARCOP]
GO
/****** Object:  StoredProcedure [LOYALTY].[sp_Clientes_Insert]    Script Date: 11/7/2019 00:41:28 ******/
SET ANSI_NULLS ON
GO
...
IF EXISTS ( SELECT * FROM [00-SRV-P].[IBridge].[SAP].[Empleados] WHERE N_DNI = @DNI )
BEGIN
...
INSERT INTO [00-SRV-P].[IBridge].[PROCESO].[Replicaciones]
(
	[Tabla]
	, [TablaClave]
	, [Accion]
...
(
	'ClientesDiarco+'
	, [IBridge].[CONFIG].[Fn_Completa_Con_Ceros] (CAST(@DNI AS VARCHAR(10)),10)
	, 'A'
	, 0
	, GETDATE()
	, 401
